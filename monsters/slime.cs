...
public class slime : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        InvokeRepeating("slimeNormal", 0f, 5f);
    }
        ...
        if(coll.gameObject.tag == "slash")  // 기본 공격에 맞았을 때
        {
            if(hp > 0f & anim.GetBool("monsterHit") == false) 
            {
                ...
                if(hp <= 0f)  // 죽었을 때
                {
                    ...
                    {
                        Instantiate(itemManager.I.banditLightBoots, transform.position, Quaternion.identity);
                    }
                    else if(drop >= 21f & drop <= 26f)
                    {
                        Instantiate(itemManager.I.buckler, transform.position, Quaternion.identity);
                    }
    ...
    IEnumerator slimeScale()  // 슬라임 행동 패턴 코루틴
    {
        for(int i = 1; i < 6; i++)
        {
            transform.localScale = new Vector3(i, i, 0f);
            transform.GetChild(0).localScale = new Vector3(1/i, 1/i, 0f);
