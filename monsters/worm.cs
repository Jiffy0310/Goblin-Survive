...
public class worm : MonoBehaviour
{
    ...
    void returnHideToNormal()
    {
        anim.SetBool("monsterHiding", false);
    }

    void HideToHiding()
    {
        anim.SetBool("monsterHiding", true);
    }
    ...
    IEnumerator wormHide()
    {
        for(;;)
        {
            if(isClose() & anim.GetBool("monsterHide") == false)
            {
                anim.SetBool("monsterHide", true);
                Invoke("HideToHiding", 2.0f);
                Invoke("returnHideToNormal", 7.0f);
