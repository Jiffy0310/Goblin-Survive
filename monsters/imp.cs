...
public class imp : MonoBehaviour
{
    ...
    public GameObject impSlashPrefab;
    ...
    bool isClose()
    {
        if (Vector3.Distance(transform.position, goblin.I.transform.position) < 3.5f) 
        {
            return true;
        }
        return false;
        ...
        for(;;)
        {
            if(isClose() & anim.GetBool("monsterAtk") == false)
            {
                ...
                if(absX>absY)
                {
                    if(X>0)
                    {
                        anim.SetFloat("monsterAtkLR", -1);  
                        anim.SetFloat("monsterAtkFB", 0);   
                    }
                    else 
                    {
                        anim.SetFloat("monsterAtkLR", 1);   
                ...
                else
                {
                    if(Y>0)
                    {
                        anim.SetFloat("monsterAtkLR", 0);  
                        anim.SetFloat("monsterAtkFB", -1);  
                    }
                    else
                    {
                        anim.SetFloat("monsterAtkLR", 0); 
                ...
                Instantiate(impSlashPrefab, new Vector3(x, y, 0f), Quaternion.identity);
                Invoke("returnAtkToNormal", 1.0f);
