...
public class arachne : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        InvokeRepeating("makeSpiderWeb", 0f, 2f);
    }
        ...
        if(coll.gameObject.tag == "slash")  // 기본 공격에 맞았을 때
        {
            if(hp > 0f & anim.GetBool("monsterHit") == false) 
            {
                ...
                if(hp <= 0f)  // 죽었을 때
                {
                    ...
                    {
                        Instantiate(itemManager.I.hunterBoots, transform.position, Quaternion.identity);
                    }
                    else if(drop > 35f & drop < 40f)  // 아이템 드랍2
                    {
                        Instantiate(itemManager.I.potion, transform.position, Quaternion.identity);
                    }
    ...
    void makeSpiderWeb()
    {
        ...
        Instantiate(spiderWebPrefab, new Vector3(x, y, 0), Quaternion.identity);
    }
}
