...
public class fireElf : MonoBehaviour
{
        ...
        if(coll.gameObject.tag == "slash")  // 기본 공격에 맞았을 때
        {
            if(hp > 0f & anim.GetBool("monsterHit") == false) 
            {
                ...
                if(hp <= 0f)  // 죽었을 때
                {
                    ...
                    {
                        Instantiate(itemManager.I.arrowTalisman, transform.position, Quaternion.identity);
                    }
                    else if(drop > 20f & drop < 25f)  // 아이템 드랍2
                    {
                        Instantiate(itemManager.I.bowTalisman, transform.position, Quaternion.identity);
                    }
                    else if(drop > 30f & drop < 35f)  // 아이템 드랍2
                    {
                        Instantiate(itemManager.I.greyKnightArmor, transform.position, Quaternion.identity);
                    }
                    else if(drop > 35f & drop < 40f)  // 아이템 드랍2
                    {
                        Instantiate(itemManager.I.salamanderSword, transform.position, Quaternion.identity);
                    }
    ...
    void returnAtkToNormal()
    {
        anim.SetBool("monsterAtk", false);
        speed = 0.7f;
    }

    bool isClose()
    {
        if (Vector3.Distance(transform.position, goblin.I.transform.position) < 4f) 
        {
            return true;
        }
        return false;
    ...
    IEnumerator fireElfAtk()
    {
        for(;;)
        {
            if(isClose() & anim.GetBool("monsterAtk") == false)
            {
                speed = 0f;
                anim.SetBool("monsterAtk", true);
                Invoke("startCR", 0.8f);
                Invoke("returnAtkToNormal", 3.8f);
    ...
    IEnumerator explosionCR()  // 폭발 코루틴
    {
        for(int i = 0; i < 5; i++) 
        {
            float x = transform.position.x;
            float y = transform.position.y;
            float xRand = Random.Range(x-3f, x+3f);
            float yRand = Random.Range(y-3f, y+3f);
            Instantiate(explosion, new Vector3(xRand, yRand, 0f), Quaternion.identity);
